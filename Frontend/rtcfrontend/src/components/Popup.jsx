...
function Popup({ popup, hidePopup }) {
  ...
  return (
    <Dialog open={showPopup} onClose={() => {}} className="relative z-10">
      <DialogBackdrop
        transition
            ...
            <form onSubmit={HandleSubmit}>
              <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 text-black">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                    ...
                    {data.type != null && data.type === "shareCode" && (
                      <div className="flex w-full p-8 justify-center items-center font-bold text-2xl">
                        {loading ? <Loader /> : <div>{shareCode}</div>}
                      </div>
                    )}

                    {data.type != null && data.type === "joinRoom" && (
                      <div>
              ...
              <div className="bg-gray-50 px-4 py-2 sm:flex sm:flex-row-reverse sm:px-6">
                {data.type !== "shareCode" ? (
                  ...
                  >
                    {!loading ? data.actionBtnText : <Loader />}
                  </button>
                ) : (
                  <></>
                )}
